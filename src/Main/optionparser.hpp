...
class Value {
  public:
    ...
    operator short() { short t; return (valid && (std::istringstream(str) >> t)) ? t : (short)0; }
    operator unsigned short() { unsigned short t; return (valid && (std::istringstream(str) >> t)) ? t : (uint16_t)0; }
